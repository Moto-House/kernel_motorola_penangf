...
#undef __BITS_GET
#define __BITS_GET(_byte, _mask, _shift) (((_byte) & (_mask)) >> (_shift))

#undef BITS_GET
#define BITS_GET(_byte, _bit) __BITS_GET(_byte, _bit, FFS(_bit))
...
#undef BITS_SET
#define BITS_SET(_byte, _bit, _val) __BITS_SET(_byte, _bit, FFS(_bit), _val)

#undef BITS_MATCH
#define BITS_MATCH(_byte, _bit) (((_byte) & (_bit)) == (_bit))
...
static int wusb3801_write_device(void *client, u32 reg, int len,
				 const void *src)
{
	const u8 *data;
	struct i2c_client *i2c = (struct i2c_client *)client;
...
static inline int wusb3801_i2c_write8(struct tcpc_device *tcpc, u8 reg,
				      const u8 data)
{
...
static int wusb3801_i2c_wr_rd(struct tcpc_device *tcpc, uint8_t reg,
			      uint8_t val)
{
	int i, ret;
	struct wusb3801_chip *chip = tcpc_get_dev_data(tcpc);
...
static void wusb3801_irq_work_handler(struct kthread_work *work)
{
	...
	if (int_sts & WUSB3801_INT_ATTACH) {
		...
		if (chip->dev_sub_id != 0xA0) {
			if (chip->cc_test_flag == 0 &&
			    BITS_GET(rc, WUSB3801_CC_STS_MASK) == 0) {
			...
			if (chip->cc_test_flag == 1) {
				...
				if (BITS_GET(rc, WUSB3801_CC_STS_MASK) == 0) {
					if (chip->cc_sts ==
					    WUSB3801_CC2_CONNECTED)
						rc = rc | 0x02;
					else if (chip->cc_sts ==
						 WUSB3801_CC1_CONNECTED)
						rc = rc | 0x01;
...
static int wusb3801_init_alert(struct tcpc_device *tcpc)
{
	...
	ret = request_irq(chip->irq, wusb3801_intr_handler,
			  IRQF_TRIGGER_LOW | IRQF_NO_THREAD | IRQF_NO_SUSPEND,
			  name, chip);
	if (ret < 0) {
...
static int mt_parse_dt(struct wusb3801_chip *chip, struct device *dev)
{
	...
	chip->irq_gpio = ret;
#else
	ret = of_property_read_u32(np, "wusb3801,irq-gpio", &chip->irq_gpio);
	if (ret < 0)
...
static int wusb3801_set_chip_state(struct wusb3801_chip *chip, uint8_t state)
{
	...
	rc = i2c_smbus_write_byte_data(
		chip->client, WUSB3801_REG_CONTROL1,
...
static int wusb3801_tcpcdev_init(struct wusb3801_chip *chip, struct device *dev)
{
	...
	if (of_property_read_u32(np, "mt-tcpc,notifier_supply_num", &val) >=
	    0) {
		if (val < 0)
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		dev_err(&client->dev,
			"failed to create dev_attr_typec_cc_orientation\n");
		ret = -ENODEV;
...
static void wusb3801_shutdown(struct i2c_client *client)
{
	...
	if (IS_ERR_VALUE_64(wusb3801_set_mode(chip, WUSB3801_SNK)) ||
	    IS_ERR_VALUE_64(wusb3801_set_chip_state(
		    chip, WUSB3801_STATE_ERROR_RECOVERY)))
...
static const struct dev_pm_ops wusb3801_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(wusb3801_i2c_suspend, wusb3801_i2c_resume)
#ifdef CONFIG_PM_RUNTIME
		SET_RUNTIME_PM_OPS(wusb3801_pm_suspend_runtime,
				   wusb3801_pm_resume_runtime, NULL)
#endif /* CONFIG_PM_RUNTIME */
};
